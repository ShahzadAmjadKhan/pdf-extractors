...
def parse_order_block_for_invoice(invoice_block, block, invoice_base_no, order_index):
    ...
    invoice_date_match = re.search(r"Invoice date: (\d{2}\.\d{2}\.\d{4})", invoice_block, re.MULTILINE)
    order_data['Invoice Date'] = invoice_date_match.group(1) if invoice_date_match else None

    # Extract Bill of Lading
    bol_match = re.search(r"Invoice reference: ([\w,]+)", block, re.MULTILINE)
    ...
    customer_match = re.search(r"^(.*?)\s+Customer number:", invoice_block, re.MULTILINE)
    customer = customer_match.group(1).strip() if customer_match else None

    address_line1_match = re.search(r"(.*?)\s+Your VAT ID no\.:", invoice_block, re.MULTILINE)
    address_line1 = address_line1_match.group(1).strip() if address_line1_match else ""

    address_line2_match = re.search(r"(.*?)\s+Email:", invoice_block, re.MULTILINE)
    address_line2 = address_line2_match.group(1).strip() if address_line2_match else ""

    address_line3_match = re.search(r"(.*?)\s+Page: \d+", invoice_block, re.MULTILINE)  # Page number is not needed for address
    ...
    customer_number_match = re.search(r"Customer number: (\d+)", invoice_block, re.MULTILINE)
    order_data['Customer Number'] = customer_number_match.group(1) if customer_number_match else None

    # Extract ETA
    eta_match = re.search(r"Loading date (\d{2}\.\d{2}\.\d{4})", block, re.MULTILINE)
    order_data['ETA'] = eta_match.group(1) if eta_match else None

    # Extract ETD
    etd_match = re.search(r"Delivery date (\d{2}\.\d{2}\.\d{4})", block, re.MULTILINE)
    order_data['ETD'] = etd_match.group(1) if etd_match else None

    # Extract Vessel/Voyage
    vessel_voyage_match = re.search(r"Vessel name:\s*(.+?)\s+Container type:", block, re.MULTILINE)
    order_data['Vessel/Voyage'] = vessel_voyage_match.group(1).strip() if vessel_voyage_match else None

    # Extract POL
    pol_match = re.search(r"Port of loading: (\w+)", block, re.MULTILINE)
    order_data['POL'] = pol_match.group(1) if pol_match else None

    # Extract POD
    pod_match = re.search(r"Port of delivery: (\w+)", block, re.MULTILINE)
    order_data['POD'] = pod_match.group(1) if pod_match else None

    # Extract NetValue (excluding currency)
    net_value_match = re.search(r"Total amount without VAT ([\d\s,]+)", block, re.MULTILINE)
    ...
    vat_match = re.search(r"([\d\s,]+) \w{3} \*", block, re.MULTILINE)
    ...
    # Extract Currency
    currency_match = re.search(r"Total amount without VAT [\d\s,]+ ([A-Z]{3})", block, re.MULTILINE)
    order_data['Currency'] = currency_match.group(1) if currency_match else "NOK"

    # Extract Reference No.
    reference_no_match = re.search(r"Order No.: (\d+)", block, re.MULTILINE)
    order_data['Reference No.'] = reference_no_match.group(1) if reference_no_match else None

    # Extract Due Date
    due_date_match = re.search(r"Due date: (\d{2}\.\d{2}\.\d{4})", invoice_block, re.MULTILINE)
    order_data['Due Date'] = due_date_match.group(1) if due_date_match else None

    # Extract Tour Number
    tour_no_match = re.search(r"Tour No.: (\d+)", block, re.MULTILINE)
...
def parse_order_block_for_container(block, invoice_base_no, order_index):
    ...
    container_no_match = re.search(r"Container no.: (\w+)", block, re.MULTILINE)
    container_data['Container ID'] = container_no_match.group(1) if container_no_match else None

    container_no_type = re.search(r"Container type: (\w+)", block, re.MULTILINE)
...
def parse_invoice_data(text):
    ...
    invoice_blocks = re.split(r"Invoice (\d+) from \d{2}\.\d{2}\.\d{4}", text)
