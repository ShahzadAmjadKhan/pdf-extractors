...
class Type2PdfParser:
    ...
    def parse_order_block_for_invoice(self, invoice_block, block, invoice_base_no, order_index, vat, word_block):
        order_data = {}
    ...
    def parse_invoice_data(self, text, word_text):
        invoices = []
        containers = []
        charges = []
        ...
        for i in range(1, len(invoice_blocks), 2):
            ...
            for order_index, order_block in enumerate(order_blocks[1:], 1):
                invoice_data = self.parse_order_block_for_invoice(invoice_blocks[0], "Order No.:" + order_block, invoice_base_no, order_index, vat, word_order_blocks[order_index])
                invoices.append(invoice_data)

                container_data = self.parse_order_block_for_container(order_block, invoice_base_no, order_index, word_order_blocks[order_index])
